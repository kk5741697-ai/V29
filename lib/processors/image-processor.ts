    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        try {
          ...
          // Safety limits
          targetWidth = Math.max(1, Math.min(this.MAX_CANVAS_SIZE, Math.floor(targetWidth)))
          targetHeight = Math.max(1, Math.min(this.MAX_CANVAS_SIZE, Math.floor(targetHeight)))
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        try {
          ...
          const sourceX = Math.max(0, (x / 100) * img.naturalWidth)
          const sourceY = Math.max(0, (y / 100) * img.naturalHeight)
          const sourceWidth = Math.max(1, (width / 100) * img.naturalWidth)
          const sourceHeight = Math.max(1, (height / 100) * img.naturalHeight)
