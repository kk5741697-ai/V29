...
export default function VCardQRCodeGeneratorPage() {
  ...
  useEffect(() => {
    generateQRCode()
  }, [vcardData, qrSize, margin, darkColor, lightColor, errorCorrection, qrStyle, eyeStyle, logoFile, logoSize])

  const generateQRCode = async () => {
    if (!vcardData.firstName.trim() && !vcardData.lastName.trim() && !vcardData.email.trim()) {
      setQrDataUrl("")
      return
    }
    ...
    try {
      const vcardString = RealQRProcessor.generateVCardQR(vcardData)
      
      const qrDataURL = await RealQRProcessor.generateQRCode(vcardString, {
        width: qrSize,
        margin,
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Failed to generate vCard QR code",
        variant: "destructive"
      })
    } finally {
  ...
  const downloadQR = (format: string) => {
    ...
    link.download = `vcard-qr-code.${format}`
    link.href = qrDataUrl
    link.click()

    toast({
      title: "Download started",
      description: `vCard QR code downloaded as ${format.toUpperCase()}`
    })
  }
      ...
      <SheetContent side="bottom" className="h-[80vh] p-0">
        ...
        <div className="p-4 border-t bg-white space-y-3">
          <Button 
            ...
            disabled={isGenerating || (!vcardData.firstName.trim() && !vcardData.lastName.trim() && !vcardData.email.trim())}
        ...
        <div className="container mx-auto px-4 py-6 lg:py-8">
          <div className="text-center mb-6 lg:mb-8">
            <div className="inline-flex items-center space-x-2 mb-4">
              <User className="h-6 w-6 lg:h-8 lg:w-8 text-purple-600" />
              <h1 className="text-2xl lg:text-3xl font-heading font-bold text-foreground">vCard QR Code Generator</h1>
            </div>
            <p className="text-base lg:text-lg text-muted-foreground max-w-2xl mx-auto px-4">
              Create QR codes for contact information. Perfect for business cards and networking events.
            </p>
          </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center text-gray-500 p-8 lg:p-16">
              <div className="relative mb-4 lg:mb-6">
                ...
                <User className="relative h-16 w-16 lg:h-20 lg:w-20 text-purple-500" />
              </div>
              <h3 className="text-xl lg:text-2xl font-semibold mb-2 lg:mb-3 text-gray-700">Configure Contact Info</h3>
              <p className="text-gray-500 mb-4 lg:mb-6 text-base lg:text-lg text-center">Enter contact details to generate QR code</p>
              <Button 
                onClick={() => setIsMobileSidebarOpen(true)}
                className="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 lg:px-8 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200"
              >
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="lg:hidden">
        <div className="bg-white border-b px-4 py-3 flex items-center justify-between shadow-sm">
          <div className="flex items-center space-x-2">
            <User className="h-5 w-5 text-purple-600" />
            <h1 className="text-lg font-semibold text-gray-900">vCard QR Generator</h1>
          </div>
          <div className="flex items-center space-x-2">
        ...
        <div className="p-4 min-h-[60vh]">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">vCard QR Code</CardTitle>
            </CardHeader>
            <CardContent>
              {qrDataUrl ? (
                <div className="relative">
                  ...
                  <div className="mt-2 text-xs text-gray-500 text-center">
                    {qrSize}×{qrSize}px • {vcardData.firstName} {vcardData.lastName}
                  </div>
                ...
                <div className="aspect-square bg-gray-100 rounded border flex items-center justify-center">
                  <div className="text-center text-gray-500">
                    ...
                    <p className="text-sm">vCard QR code will appear here</p>
                  </div>
                </div>
              )}
            </CardContent>
      ...
      <div className="hidden lg:flex h-[calc(100vh-8rem)] w-full overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="bg-white border-b px-6 py-3 flex items-center justify-between shadow-sm flex-shrink-0">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <User className="h-5 w-5 text-purple-600" />
                <h1 className="text-xl font-semibold text-gray-900">vCard QR Code Generator</h1>
              </div>
            ...
            {qrDataUrl ? (
              ...
              <div className="text-center max-w-md mx-auto">
                <div className="relative mb-8">
                  <div className="absolute inset-0 bg-purple-500/20 rounded-full blur-xl"></div>
                  <User className="relative h-24 w-24 text-purple-500 mx-auto" />
                </div>
                <h3 className="text-2xl font-semibold mb-3 text-gray-700">Create Contact QR Code</h3>
                <p className="text-gray-500 mb-6 text-lg">
                  Configure your contact details to generate QR code
                </p>
              </div>
            )}
          </div>
        ...
        <div className="w-80 xl:w-96 bg-white border-l shadow-lg flex flex-col h-full">
          <div className="px-6 py-4 border-b bg-gray-50 flex-shrink-0">
            <div className="flex items-center space-x-2">
              <User className="h-5 w-5 text-purple-600" />
              <h2 className="text-lg font-semibold text-gray-900">Contact Configuration</h2>
            </div>
            <p className="text-sm text-gray-600 mt-1">Configure contact info and QR style</p>
          </div>
          ...
          <div className="p-6 border-t bg-gray-50 space-y-3 flex-shrink-0">
            ...
            {/* Contact Info */}
            {(vcardData.firstName || vcardData.lastName || vcardData.email) && (
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                <h4 className="text-sm font-semibold text-purple-800 mb-2">Contact Info</h4>
                <div className="text-xs text-purple-700 space-y-1">
                  {(vcardData.firstName || vcardData.lastName) && (
                    <div className="flex justify-between">
                      <span>Name:</span>
                      <span className="font-medium">{vcardData.firstName} {vcardData.lastName}</span>
                    </div>
                  )}
                  {vcardData.organization && (
                    <div className="flex justify-between">
                      <span>Company:</span>
                      <span className="font-medium">{vcardData.organization}</span>
                    </div>
                  )}
                  {vcardData.email && (
                    <div className="flex justify-between">
                      <span>Email:</span>
                      <span className="font-medium">{vcardData.email}</span>
                    </div>
                  )}
                  {vcardData.phone && (
                    <div className="flex justify-between">
                      <span>Phone:</span>
                      <span className="font-medium">{vcardData.phone}</span>
                    </div>
                  )}
                </div>
              </div>
